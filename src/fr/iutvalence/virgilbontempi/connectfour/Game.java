...
public class Game {
	...
	private void playARound(Player currentPlayer) {
		try {
			...
			System.out
					.println("Input an other column, the selected column is out of range");
			playARound(currentPlayer);
		} catch (FullColumnException f) {
			System.out
					.println("Input an other column, the selectedcolumn is full");
			playARound(currentPlayer);
		}
	}
	...
	public void placementPiece(int column) {
		int line = 0;
		while (line < Grid.NBLINE
				&& this.grid.getGrille()[line][column] != Piece.EMPTY) {
			line++;
		}
	...
	public boolean columnIsFull(int column) {
		if (this.grid.getGrille()[0][column] != Piece.EMPTY)
			return true;
		return false;
	}
	...
	private boolean horizontalRight(int line, int column) throws EmptyPieceException{
		...
		return ((this.grid.getGrille()[line][column] == this.grid.getGrille()[line][column + 1])
				&& (this.grid.getGrille()[line][column + 2] == this.grid.getGrille()[line][column + 3]) && (this.grid
					.getGrille()[line][column + 1] == this.grid.getGrille()[line][column + 2]));
	}
	...
	private boolean verticalDown(int line, int column) throws EmptyPieceException{
		...
		return ((this.grid.getGrille()[line][column] == this.grid.getGrille()[line - 1][column])
				&& (this.grid.getGrille()[line - 2][column] == this.grid.getGrille()[line - 3][column]) && (this.grid
					.getGrille()[line - 1][column] == this.grid.getGrille()[line - 2][column]));
	}
	...
	private boolean diagonalDownRight(int line, int column) throws EmptyPieceException{
		...
		return ((this.grid.getGrille()[line][column] == this.grid.getGrille()[line + 1][column + 1])
				&& (this.grid.getGrille()[line + 2][column + 2] == this.grid.getGrille()[line + 3][column + 3]) && (this.grid
					.getGrille()[line + 1][column + 1] == this.grid.getGrille()[line + 2][column + 2]));
	}
